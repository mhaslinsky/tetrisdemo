...
/**
 * Individual block component for hold preview rendering
 * Memoized to prevent unnecessary re-renders
 */
const PreviewBlock: React.FC<PreviewBlockProps> = React.memo(({ type, disabled = false, className = "" }) => {
  ...
  return (
    <div
      ...
      data-testid={`hold-block-${type}${disabled ? "-disabled" : ""}`}
    />
  );
});

/**
 * Component for displaying the held tetromino piece in a preview grid
 * Memoized to prevent unnecessary re-renders when piece or canHold hasn't changed
 */
export const HoldPiecePreview: React.FC<HoldPiecePreviewProps> = React.memo(
  ({ piece, canHold, className = "" }) => {
    if (!piece) {
      return (
        <div
          className={`w-16 h-16 bg-gray-900 rounded border border-gray-600 ${className}`}
          data-testid='hold-piece-empty'
          role='img'
          aria-label='No piece held. Press C or Shift to hold the current piece.'
        >
          <div className='flex items-center justify-center h-full text-gray-500 text-xs'>Empty</div>
        </div>
      );
    }
    ...
    return (
      <div
        ...
        data-testid='hold-piece-preview'
        role='img'
        aria-label={`Held piece: ${piece.type} tetromino${
          !canHold ? " (cannot hold again until next piece)" : ""
        }`}
      >
        <div
        ...
        {!canHold && (
          <div className='absolute inset-0 flex items-center justify-center'>
            <div className='text-xs text-red-400 font-semibold bg-gray-800 px-1 rounded'>USED</div>
          </div>
        )}
      </div>
    );
...
HoldPiecePreview.displayName = "HoldPiecePreview";
